    ...
    public partial class frmAddLabOrder : Form
    {
        ...
        private void activateSatelliteCombo()
        {
            ...
            if (emrType == "iqcare")
            {
               ...
               /* string sqlquery = "select usr.UserID,LTRIM(RTRIM(usr.UserLastName + '  ' + usr.UserFirstName + ' ' + '-' + ' ' + md.Name)) as EmployeeName
                    
                    ,usr.Designation,md.Name from mst_User usr inner join mst_Designation md on md.Id = usr.Designation " +
                    " where usr.DeleteFlag = 0";*/
                string sqlquery = "SELECT md.UserID,LTRIM(RTRIM(md.UserLastName)) + ' ' + LTRIM(RTRIM(md.UserFirstName)) AS EmployeeName,LTRIM(RTRIM(mdes.NAME)) AS Designation, " +
               " md.DeleteFlag FROM  mst_User md INNER JOIN mst_Employee me  on me.EmployeeID=md.EmployeeId INNER JOIN mst_Designation mdes on mdes.Id=me.DesignationID  " +
               " where md.DeleteFlag is null or md.DeleteFlag=0";
        ...
        private void btnCreateLab_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if((String.IsNullOrEmpty(Convert.ToString(FacilityID))==true)|| FacilityID =='0')
                {
                    string emrConnString2 = iqcareconnectionstring.ToString();
                    int pk= Convert.ToInt32(clsGbl.PatientID);
                 string FacilityQuery=string.Format("select LocationID from mst_Patient where Ptn_pk ='{0}'", pk);
                  DataTable facility = (DataTable)en.ReturnObject(emrConnString2, ClsUtility.theParams
                , FacilityQuery, ClsUtility.ObjectEnum.DataTable, serverType);
                 FacilityID = Convert.ToInt32(facility.Rows[0].ItemArray[0].ToString());
               ...
               // mycomm.Parameters.Add(new SqlParameter("@LabID", labid));
                ...
                mycomm.Parameters.Add(new SqlParameter("@ReportedByDate", labdoneon));
                ...
                mycommUpdate.Parameters.Add(new SqlParameter("@LabOrderId", '0'));
                ...
                mycommUpdate.Parameters.Add(new SqlParameter("@urgent", '0'));
                mycommUpdate.Parameters.Add(new SqlParameter("@Justification",justification));
                mycommUpdate.Parameters.Add(new SqlParameter("@ReportedByDate", labdoneon));
               // mycommUpdate.Parameters.Add(new SqlParameter("@ReportBy", '0'));
               ...
               // mycommUpdate.Parameters.Add(new SqlParameter("@Confirmedby", '0'));
...

                {
                    ...
                    string query = ViralLoadQuery.VLKHHLabList.ToString();
